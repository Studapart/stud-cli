...
class ConfigShowHandlerTest extends CommandTestCase
{
    ...
    public function testHandleReturnsSuccessWithProjectConfigWhenInRepo(): void
    {
        ...
        $this->gitRepository->expects($this->once())
            ...
            ->willReturn(ConfigFileReadResult::readable(['projectKey' => 'PROJ', 'githubToken' => 'gh-secret']));
    ...
    public function testHandleTreatsEmptyProjectConfigAsNull(): void
    {
        ...
        $this->gitRepository->expects($this->once())
            ->method('readProjectConfigResult')
            ->willReturn(ConfigFileReadResult::missing());
    ...
    public function testHandleAddsWarningWhenGlobalConfigCannotBeParsed(): void
    {
        $this->fileSystem->expects($this->once())
            ->method('fileExists')
            ->willReturn(true);
        ...
        $this->assertSame(ResponseMessageLevel::Warning, $response->getWarnings()[0]->level);
        $this->assertSame('config.show.global_config_unreadable', $response->getWarnings()[0]->message->key);
        $this->assertSame('Parse error', $response->getWarnings()[0]->technicalDetails);
    }

    public function testHandleAddsWarningWhenProjectConfigCannotBeParsed(): void
    {
        $this->fileSystem->expects($this->once())
            ->method('fileExists')
            ->willReturn(true);
        ...
        $this->gitRepository->expects($this->once())
            ->method('readProjectConfigResult')
            ->willReturn(ConfigFileReadResult::unreadable('invalid yaml'));
        ...
        $this->assertNull($response->projectConfig);
        $this->assertCount(1, $response->getWarnings());
        $this->assertSame('config.show.project_config_unreadable', $response->getWarnings()[0]->message->key);
        $this->assertSame('invalid yaml', $response->getWarnings()[0]->technicalDetails);
    }
    ...
    public function testHandleWithKeyInWhitelistPresentInProjectReturnsSingleKeySectionProject(): void
    {
        ...
        $this->gitRepository->expects($this->once())
            ->method('readProjectConfigResult')
            ->willReturn(ConfigFileReadResult::readable(['JIRA_DEFAULT_PROJECT' => 'PROJ']));
