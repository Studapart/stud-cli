...
1.  **Ingest & Verify**: Use `stud sh <JiraWorkItemKey>` to verify the ticket exists and to understand its requirements. If the ticket cannot be found, halt the process and report an error. You must carefully read the ticket's description, acceptance criteria, and any linked documentation. **Requirements vs conventions**: Interpret requirements so the delivered outcome satisfies the ticket while conforming to CONVENTIONS and ADRs. If the ticket suggests an approach that conflicts (e.g. "show X on the CLI" without specifying how), choose an implementation that meets the acceptance criteria and adheres to the compliance gate (§2); do not default to the quickest code path if it violates Responder/PageViewConfig/Logger or other rules.
    ...
    **Confluence supplementary context (optional):** After reviewing the issue, scan the description (and any other plain-text fields you rely on for requirements) for **Confluence page URLs** so full wiki specs can inform planning without opening the browser manually. `stud confluence:show` (alias `stud csh`) already supports `--url` and agent JSON; success payloads include markdown in `data.body` (see `echo '{}' | stud help --agent` for `confluence:show` input: `url`, `page`, `confluenceUrl`).

    - **Detection:** Treat a string as a candidate wiki page URL when it is a well-formed `http`/`https` URL whose path includes both `/wiki/` and `/pages/` (typical Atlassian Cloud page links, e.g. paths like `/wiki/spaces/SPACE/pages/123456/...`). Deduplicate: fetch each distinct URL at most once. If no such URL appears, skip this subsection.
    - **Fetch (agent):** For each distinct URL, run `echo '{"url":"<url>"}' | stud csh --agent` with proper JSON escaping in the shell or a here-doc. If the Confluence base URL differs from your configured default, use the `confluenceUrl` property in the agent JSON as needed (same schema as above).
    ...
    - **Failure modes (fail-soft; optional context only):** Missing URL in the description, agent validation errors, Confluence API errors, rate limiting, or missing permission must **not** block the protocol. Note the failure in your plan or working notes and continue using the Jira text and codebase only.
    - **Edge cases:** Descriptions may use ADF; links might not appear as plain URLs (manual copy or follow-up may be needed). Multiple URLs are supported by fetching each distinct one. If rate limits are a concern, serialize requests or backoff between calls.
...
2.  **Submit Pull Request**: Use agent mode with **`stageAll` when the working tree is not yet committed** so one command performs commit + push + PR (equivalent to `stud push` then submit preflight/PR): e.g. `echo '{"labels": "AI-Generated,RFR","stageAll": true}' | stud submit --agent`. Optional JSON keys `isNew`, `message`, and `pleaseFallback` match `stud push --agent` when `stageAll` is true. If you already committed and only need to open/update the PR, omit `stageAll` and use e.g. `echo '{"labels": "AI-Generated,RFR"}' | stud submit --agent`. This uses the default base branch and provider, applies the labels, and returns structured JSON. This step MUST only be performed after all previous phases, including testing, documentation, and integrity checks, are complete.
